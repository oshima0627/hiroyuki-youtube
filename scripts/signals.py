...
AVOID: dict[str, tuple[str, ...]] = {
    ...
    "政治": ("選挙", "総裁選", "自民党", "立憲", "議員", "総理", "首相", "内閣",
             ...
             "親権", "憲法", "外交", "移民", "難民", "与党", "野党", "投票",
             "NHK", "皇族", "愛子",
             # 地政学。恋愛の回に「中国にとって…反アメリカの国が減る」が入った
             "大国", "地政学", "同盟", "安全保障", "制裁", "敵に回す", "軍"),
    # 「氏」はトピックタイトルではほぼ確実に実名の公人を指す。
    # **実名の人物が素通りした**（「泉さんの『お薬』リスト」「石丸さんの絶対条件」）。
    # 一般語で捕まえられないので、この配信に出てくる名前を並べる。
    # 増え続けるので、候補一覧のタイトルは必ず人が読むこと
    "個人": ("社長", "会長", "氏", "さんは無能", "退任", "決算", "不祥事", "逮捕",
             "石丸", "泉さん", "泉房穂", "夏野", "山本太郎", "本田", "石川",
             "桑田", "須田", "音喜多", "かっぴー", "HIKAKIN", "ヒカキン",
             "ホリエモン", "堀江", "西村ゆか", "岡田", "斗司夫", "悠人",
             "吉本", "カジノ", "賭博", "違法", "炎上", "訴訟", "復讐", "ガーシー"),
    ...
    "センシティブ": ("自殺", "死にたい", "性病", "セックス", "風俗", "レイプ",
                     ...
                     # 精神疾患・発達障害・死亡事例。当人の話でなくても扱いが難しい
                     "統合失調症", "ADHD", "発達障害", "精神疾患", "パニック",
                     "毒", "死んだ", "亡くな", "遺体", "事故死", "孤独死", "薬",
                     # 属性への一般化。「文系は金稼がないからね」が入った
                     "文系は", "理系は", "女は", "男は", "老害", "無能"),
...
# 検索される固有名詞。**タイトルに置いたときに実際に打たれる語だけを入れる。**
#
# 起点は思いつきではなく実測。ショート「簿記1級を取るか税理士を目指すか」は
# 視聴の 76.5% が YouTube 検索から来ており、検索語は
# 税理士試験 30.8% / 簿記1級 23.1% / 日商簿記1級 15.4% だった（2026-08-25、Studio）。
# ここを起点に、Studio の検索語レポートで増やしていく。
#
# **一般語を入れないこと。** 「仕事」「お金」「転職」のような語は検索ボリュームは
# 大きいが競合が強く、0登録者のチャンネルでは取れない。制度名・資格名・職種名の
# ように、打つ人が既に答えを探している語だけを入れる。
SEARCH: dict[str, tuple[str, ...]] = {
    "資格": ("簿記", "日商簿記", "税理士", "公認会計士", "中小企業診断士",
             "社労士", "社会保険労務士", "宅建", "行政書士", "司法書士",
             "司法試験", "FP", "ファイナンシャルプランナー", "TOEIC", "英検",
             "基本情報", "応用情報", "情報処理", "看護師", "保育士", "介護福祉士",
             "教員免許", "調理師", "美容師", "電気工事士", "危険物"),
    "制度": ("NISA", "iDeCo", "確定拠出年金", "年金", "国民年金", "厚生年金",
             "失業保険", "雇用保険", "扶養", "確定申告", "青色申告",
             "住宅ローン", "奨学金", "児童手当", "医療費控除", "ふるさと納税",
             "傷病手当", "労災", "健康保険", "源泉徴収"),
    "職種": ("公務員", "派遣", "契約社員", "正社員", "フリーランス", "個人事業主",
             "自営業", "営業職", "エンジニア", "プログラマー", "システムエンジニア",
             "介護職", "事務職", "工場勤務", "トラック運転手", "美容師", "教師",
             "銀行員", "経理", "人事", "コンサル"),
    "働き方": ("転職エージェント", "退職代行", "副業", "起業", "独立", "在宅勤務",
               "リモートワーク", "夜勤", "残業代", "有給", "育休", "産休",
               "パワハラ", "セクハラ", "休職"),
    # 実測で2問組めることを確かめてから足した語（2026-08-25）。
    # 1件しか出ない語は素材が増えてからにする。語彙は増やすほど良いものではない
    "住まい": ("不動産", "賃貸", "持ち家"),
    "相場": ("為替", "円安", "円高", "米国株", "長期金利"),
    "学校": ("大学院", "専門学校", "通信制", "偏差値", "MARCH", "早慶", "国公立",
             "中学受験", "高校受験", "大学受験", "浪人", "留年", "推薦入試",
             "総合型選抜", "編入", "留学", "不登校"),
}

# **AVOID と衝突する語を SEARCH に入れないこと。**
# 「生活保護」「薬剤師」「うつ病」「適応障害」を入れていたが、それぞれ AVOID の
# 「生活保護」「薬」「うつ」「障害」に当たるので、タイトルに出た瞬間に risk が立って
# 必ず落ちる。**登録されているのに構造上0件しか出せない語**になっていた（2026-08-25 実測）。
# 語彙を足すときは search_avoid_conflicts() で確かめること。
# AVOID を緩めて通す方向には倒さない。AVOID は素通りした事故の積み重ねでできている。


def search_avoid_conflicts() -> list[tuple[str, str]]:
    """SEARCH の語のうち AVOID に当たるものを返す。[(SEARCH語, AVOID語), ...]"""
    out = []
    for ws in SEARCH.values():
        for w in ws:
            for aws in AVOID.values():
                for aw in aws:
                    if aw in w:
                        out.append((w, aw))
    return out
...
def search_marks(cues: list[dict]) -> list[dict]:
    """検索される固有名詞の出現位置。[{"seconds","kind","word","line"}, ...]"""
    return _scan(cues, SEARCH)
