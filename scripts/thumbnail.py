...
def pick_frame(video: Path, out_dir: Path, count: int = 9) -> tuple[Path, float, float]:
    ...
    **candidates を増やしても横顔は避けられない。** 8/14・8/15・8/16 の3本が
    伏し目・横顔になっていたので候補を9→40に増やして選び直したが、
    出てきたのは後頭部と横顔で、9点のときより悪くなった（2026-08-25 に実測）。
    face_score は頭部下半分の肌色比率なので、**横顔でも高く出る**のが原因で、
    密度を上げても順位が変わらない。正面かどうかを見る指標に替えない限り
    ここは直らない。candidates は「どこを見たか」を変えるだけの逃げ道として残す。
    """
